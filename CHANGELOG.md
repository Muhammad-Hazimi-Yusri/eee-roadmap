...
**Grid Code Viewer hardening — test coverage + user-facing error handling**

> Note: the changelog was not maintained between 0.22.15 and 0.27.4 (the Grid Code & Standards Viewer milestone shipped via PRs #17–#24); that history lives in git.

### Added

- **`reindex.ts` orchestrator tests** (`src/lib/grid-code/__tests__/reindex.test.ts`, 11 tests) — phase ordering, version-stamp-last, clause filtering, the no-clauses branch, both failure modes (partial-write-preserved contract), in-flight de-duplication + clear-on-settle, progress forwarding, timeout passthrough, and timeout aggregation. `reindex.ts` coverage 0% → 97%.
- **`pdf-extract.ts` tier tests** — exported the document-taking helpers (`selectDocumentOutline`, `resolveIdsToPages`, `extractEmbeddedOutline`, `extractTocOutline`, `extractHeadingLines`, `resolveDestPage`, `splitOutlineTitle`/`cleanOutlineTitle`, `parseTocStructureLine`) behind a behaviour-preserving seam, then added 33 tests against a fake PDF document (tier selection, dest resolution, embedded-outline walking, ToC/heading tiers, timeout paths). Branch coverage ~38% → 85%; `lib/grid-code` folder 50% → 83%.
- **Upload-flow e2e** (`tests/integration/grid-code-upload.spec.ts`) — drives a real browser through upload → extract → outline plus the new error toasts, backed by committed fixtures (`tests/fixtures/grid-code-sample.pdf` generated by `make-sample-pdf.mjs`, and a not-a-PDF fixture).
- **`IndexingResult`** returned from `runDocIndexing` (`{ clausesIndexed, outlineEntries, timedOut, stamped }`) plus an `onTimeout` hook threaded through every extractor timeout site, so callers can tell a complete result from a truncated one.
- **`isQuotaError`** helper (`local-pdfs.ts`) for detecting IndexedDB quota exhaustion.

### Changed

- **Surfaced previously-silent failures to the user** through the existing toast layer:
  - **Upload** (`UploadDropzone.tsx`): pre-flight storage-quota warning (`navigator.storage.estimate`); quota-aware store-failure message; background-indexing failure and timeout now raise toasts instead of only logging to the console.
  - **Viewer** (`ViewerSplitPane.tsx`): auto and manual re-index failures raise error toasts; a timed-out (partial) re-index raises an "outline may be incomplete" notice.

### Fixed

- **Unhandled promise rejection on indexing failure** — `runDocIndexing` chained `run.finally(cleanup)`, which forked a second promise that re-threw on rejection, producing an unhandled rejection on every failed extraction (in addition to the caller-handled one). Replaced with a settle-only `then(clear, clear)` cleanup (`reindex.ts`).
